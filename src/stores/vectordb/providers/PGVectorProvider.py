...
from ..vectorDBEnums import (DistanceMethodEnums, PGVectorTableschemaEnums,
                              PGVectorDistanceMethodEnums, PGVectorIndexTypeEnums, VectorDBEnums)
import logging
from typing import List
from models.db_schemes.minirag import RetrievedDocument
...
class PGVectorProvider(VectorDBInterface):
    ...
    async def list_all_collections(self) -> list:
        ...
        async with self.db_client() as session:
            async with session.begin():
                list_tbl = sql_text("SELECT tablename FROM pg_tables WHERE tablename LIKE :_prefix")
                results = await session.execute(list_tbl, {"_prefix": self.pgvector_table_prefix})
    ...
    async def create_vector_index(self, collection_name:str, index_type: str = PGVectorIndexTypeEnums.HNSW.value):
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                self.logger.info(f"START: creating vector index for collecton: {collection_name}")
                ...
                self.logger.info(f"End: creating vector index for collecton: {collection_name}")
    ...
    async def insert_one(self, collection_name: str,
        ...
        is_collection_existed = await self.is_collection_existed(collection_name = collection_name)
        if not is_collection_existed:
            self.logger.error(f"cant insert into non-existing collection {collection_name}")
            return False

        if not record_id:
            self.logger.error(f"can not insert record without record_id {collection_name}")
            return False
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                insert_sql = sql_text(
                    ...
                    f'VALUES (:record_id, :text, :vector, :metadata)'
                )
    ...
    async def search_by_vector(self, collection_name: str,
        ...
        is_collection_existed = await self.is_collection_existed(collection_name = collection_name)
        if not is_collection_existed:
            self.logger.error(f"cant search in non-existing collection {collection_name}")
            return False
        ...
        async with self.db_client() as session:
            async with session.begin():
                self.logger.info(f"Searching records in collection: {collection_name}")

            search_sql = sql_text(
                        f'SELECT {PGVectorTableschemaEnums.TEXT.value} AS text, '
                        f'1 - ({PGVectorTableschemaEnums.VECTOR.value} <-> :query_vector) AS score '
